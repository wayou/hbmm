...
function onList() {
    getCurrentRegistry(function (cur) {
        ...
        Object.keys(allMirrors).forEach(function (key) {
            ...
            info.push(prefix + key + line(key, 8) + item.brew);
        });
...
function onUse(name) {
    ...
    if (allMirrors.hasOwnProperty(name)) {
        ...
        runCommand(`cd "$(brew --repo)" && git remote set-url origin ${mirror.brew}`)
        runCommand(`cd "$(brew --repo)/Library/Taps/homebrew/homebrew-core" && git remote set-url origin ${mirror.core}`, () => {
            printMsg([
                '', '   Registry has been set to: ' + name, ''
            ]);
        })
    } else {
        printMsg([
            '', name + ' not found!', 'Check the spell and try again.'
        ]);
    }
}

function getCurrentRegistry(cb) {
